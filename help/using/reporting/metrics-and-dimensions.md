...
title: 度量和維度
description: 瞭解Journey Orchestration可用的維度和量度
feature: Journeys
role: User
level: Intermediate
...
>[!NOTE]
>
>僅在您有此傳遞時，才會填入Adobe Campaign Standard數據。
...
| **欄位群組** | 用來擴充歷程中資料的欄位群組清單（**欄位群組名稱**），例如，設定檔欄位群組、Geometrixx訂房系統。 |
...
| **瀏覽器** | 開啟或點選訊息的來源瀏覽器。 |
| **傳送名稱** | 傳遞的加標籤和 ID。 |
| **裝置** | 開啟/檢視/點按電子郵件/SMS 簡訊/推送通知的裝置。 |
...
| **追蹤URL** | 使用者從訊息中點按的URL。 |
| **追蹤URL類別** | 指派給追蹤URL的類別。 |
| **追蹤URL標籤** | 加標籤提供給URL，例如鏡像頁面，聯繫我們或打開。 |
| **變體** | A/B測試時的電子郵件變體。 |
...
| **當前費率** | 目前在歷程中的總人數與進入歷程的人數之比較。 |
| **進入** | 在歷程中開始個別專案時所發生的事件總數。 |
...
| 量度 | 定義 |
|--- |--- |
| **在塊清單上** | 已將電子郵件宣告為垃圾郵件的收件者數目。 |
| **塊清單率** | 與已傳送的訊息相較之封鎖清單上的訊息總數。 |
| **退回+錯誤** | 傳遞和自動傳回處理期間累積的錯誤總數與已傳送之訊息總數的關係。 |
| **退回+錯誤率** | 與已傳送的訊息相比退回的訊息總數。 |
| **按一下** | 內容在傳遞中被點按的次數。 |
| **點進率** | 相較於已傳遞訊息數量的傳遞中點按總數。 |
| **已傳遞** | 成功傳送的訊息數（與已傳送訊息總數相關）。 |
| **傳遞率** | 與已發送郵件相比，成功傳遞的消息總數。 |
| **錯誤** | 歷程中發生但並未阻止歷程成功的錯誤總數。 |
| **硬跳出** | 永久性錯誤的總數，例如錯誤的電子郵件位址。 |
