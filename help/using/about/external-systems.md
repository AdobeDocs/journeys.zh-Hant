...
若要進一步了解上限API以及如何設定上限規則，請參閱 [本頁](../api/capping.md).
...
預設沒有上限規則。 上限規則是在沙箱層級使用上限設定API為特定端點（稱為的URL）定義。 請參閱 [本節](../about/external-systems.md#capping) 和 [本頁](../api/capping.md).
...
在每個歷程中，您可以定義逾時持續時間。 逾時期間會在歷程的屬性中設定。 逾時期間必須介於1秒和30秒之間。 請參閱 [本節](../about/external-systems.md#timeout) 和 [本頁](../building-journeys/changing-properties.md#timeout_and_error).